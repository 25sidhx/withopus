...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_form = bool(re.search(r'<form|<input|password|credit|card|payment', content, re.IGNORECASE)) and not filepath.endswith('.css')
        ...
        if nav_blocks:
            nav_content = "".join(nav_blocks)
            nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', nav_content, re.IGNORECASE))
        else:
            header_blocks = re.findall(r'<header[^>]*>(.*?)</header>', content, re.DOTALL | re.IGNORECASE)
            if header_blocks:
                header_content = "".join(header_blocks)
                nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', header_content, re.IGNORECASE))
            else:
                nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', content, re.IGNORECASE))
                if 'footer' in content.lower() or 'sections' in content.lower():
                    nav_items = min(nav_items, 5)
        ...
        if has_scroll_driven and filename.endswith(('.js', '.jsx', '.tsx', '.ts')):
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {
                'node_modules', '.git', 'dist', 'build', '.next', '.agent', '.shared',
                'antigravity-kit-main', 'antigravity-skills', 'antigravity-skills-main',
                'spikes', 'agency-agents-', 'opus-autopilot', 'test_post', 'leads',
                '.planning', 'claude code', '.netlify'
            }]
            for file in files:
                if file in {'opus-brand-guidelines.html', 'stitch-preview.html', 'brand-kit.html'}:
                    continue
